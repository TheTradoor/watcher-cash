...
var (
	bundle     *withdraw.ProverBundleV2
	registered []js.Func
)
...
func jsonResult(value any) string {
	...
	if err != nil {
		return `{"error":"failed to encode V2 WebAssembly prover response"}`
	}
...
func loadBundle(_ js.Value, arguments []js.Value) any {
	if len(arguments) != 1 || arguments[0].Type() != js.TypeObject {
		return errorResult(fmt.Errorf("watcherProverV2LoadBundle expects one asset object"))
	}
	...
	loaded, err := withdraw.LoadProverBundleBytesV2(assets)
	if err != nil {
		return errorResult(err)
	}
	bundle = loaded
	return jsonResult(map[string]any{
		...
		"circuits":     []string{"deposit-v2", "withdraw-v2"},
	})
}

func proveDeposit(_ js.Value, arguments []js.Value) any {
	if bundle == nil {
		return errorResult(fmt.Errorf("V2 browser proving bundle is not loaded"))
	}
	if len(arguments) != 1 || arguments[0].Type() != js.TypeString {
		return errorResult(fmt.Errorf("watcherProverV2ProveDeposit expects one JSON string"))
	}
...
func proveWithdraw(_ js.Value, arguments []js.Value) any {
	...
	if len(arguments) != 1 || arguments[0].Type() != js.TypeString {
		return errorResult(fmt.Errorf("watcherProverV2ProveWithdraw expects one JSON string"))
	}
...
func main() {
	register("watcherProverV2LoadBundle", loadBundle)
	register("watcherProverV2ProveDeposit", proveDeposit)
	register("watcherProverV2ProveWithdraw", proveWithdraw)
	js.Global().Set("watcherProverV2RuntimeReady", true)
	select {}
}
