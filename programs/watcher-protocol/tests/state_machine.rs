...
use watcher_protocol_program::{
    verify_withdrawal_proof, DepositRecord, ProtocolConfig, ProtocolStateModel, WatcherError,
    WithdrawalStatement,
};

fn commitment(x: u8) -> [u8; 32] {
    [x; 32]
}
fn nullifier(x: u8) -> [u8; 32] {
    [x; 32]
}
...
fn development_config_refuses_protocol_fee() {
    ...
    assert_eq!(
        config.validate(),
        Err(WatcherError::FeesDisabledDuringDevelopment)
    );
}
...
fn rejects_zero_deposit() {
    let mut state = ProtocolStateModel::default();
    assert_eq!(
        state.record_deposit(DepositRecord {
            commitment: commitment(1),
            amount: 0
        }),
...
fn double_spend_is_rejected() {
    ...
    assert_eq!(
        state.apply_verified_withdrawal(second),
...
fn duplicate_nullifiers_inside_one_withdrawal_are_rejected() {
    ...
    assert_eq!(
        state.apply_verified_withdrawal(w),
