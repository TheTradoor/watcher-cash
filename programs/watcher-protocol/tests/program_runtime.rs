...
fn initialize_deposit_and_failed_withdrawal_do_not_mutate_nullifiers() {
    ...
    let mut la=1;let mut lc=1;let mut lcm=1;let mut ln=1;
    let mut authority_data=[];let mut config_data=vec![0u8;CONFIG_ACCOUNT_LEN];let mut commitments_data=vec![0u8;5+32*8];let mut nullifiers_data=vec![0u8;5+32*8];
    {
        let accounts=vec![
            program_account(&authority,&program_id,&mut la,&mut authority_data,true),
            program_account(&config_key,&program_id,&mut lc,&mut config_data,true),
            program_account(&commitments_key,&program_id,&mut lcm,&mut commitments_data,true),
            program_account(&nullifiers_key,&program_id,&mut ln,&mut nullifiers_data,true),
        ];
    ...
    {
        let accounts=vec![program_account(&config_key,&program_id,&mut lc,&mut config_data,true),program_account(&commitments_key,&program_id,&mut lcm,&mut commitments_data,true)];
    ...
    let before=nullifiers_data.clone(); let recipient=Pubkey::new_unique();
    let err={
        let accounts=vec![program_account(&config_key,&program_id,&mut lc,&mut config_data,true),program_account(&commitments_key,&program_id,&mut lcm,&mut commitments_data,true),program_account(&nullifiers_key,&program_id,&mut ln,&mut nullifiers_data,true)];
        process_instruction(&program_id,&accounts,&withdraw_data([11u8;32],[12u8;32],[22u8;32],&recipient)).unwrap_err()
    };
    assert_eq!(err,ProgramError::Custom(WatcherError::ProofVerificationUnavailable as u32+1));
    assert_eq!(nullifiers_data,before,"failed proof must not burn nullifiers");
    assert!(!contains_32(&commitments_data,&[22u8;32]).unwrap(),"failed proof must not append change commitment");
}
