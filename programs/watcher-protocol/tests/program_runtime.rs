...
use watcher_protocol_program::{codec::{contains_32, ConfigAccount, CONFIG_ACCOUNT_LEN}, process_instruction, WatcherError, STATE_VERSION};
...
fn set_root_data(root:[u8;32])->Vec<u8>{let mut v=vec![3];v.extend_from_slice(&root);v}
...
fn initialized_config(program_id:&Pubkey,authority:&Pubkey,treasury:&Pubkey,config_key:&Pubkey)->(u64,Vec<u8>){
 let commitments_key=Pubkey::new_unique();let nullifiers_key=Pubkey::new_unique();let mut la=1;let mut lc=1;let mut lcm=1;let mut ln=1;let mut ad=[];let mut cd=vec![0u8;CONFIG_ACCOUNT_LEN];let mut cmd=vec![0u8;69];let mut nd=vec![0u8;69];
 {let accounts=vec![program_account(authority,program_id,&mut la,&mut ad,true),program_account(config_key,program_id,&mut lc,&mut cd,true),program_account(&commitments_key,program_id,&mut lcm,&mut cmd,true),program_account(&nullifiers_key,program_id,&mut ln,&mut nd,true)];process_instruction(program_id,&accounts,&init_data(treasury)).unwrap();}
 (lc,cd)
}

#[test]
fn authority_can_set_nonzero_trusted_merkle_root(){
 let program_id=Pubkey::new_unique();let authority=Pubkey::new_unique();let treasury=Pubkey::new_unique();let config_key=Pubkey::new_unique();let(mut lc,mut cd)=initialized_config(&program_id,&authority,&treasury,&config_key);let mut la=1;let mut ad=[];let root=[42u8;32];
 {let accounts=vec![program_account(&authority,&program_id,&mut la,&mut ad,true),program_account(&config_key,&program_id,&mut lc,&mut cd,true)];process_instruction(&program_id,&accounts,&set_root_data(root)).unwrap();}
 assert_eq!(ConfigAccount::unpack(&cd).unwrap().merkle_root,root);
}

#[test]
fn trusted_merkle_root_update_requires_configured_authority(){
 let program_id=Pubkey::new_unique();let authority=Pubkey::new_unique();let attacker=Pubkey::new_unique();let treasury=Pubkey::new_unique();let config_key=Pubkey::new_unique();let(mut lc,mut cd)=initialized_config(&program_id,&authority,&treasury,&config_key);let before=cd.clone();let mut la=1;let mut ad=[];
 let err={let accounts=vec![program_account(&attacker,&program_id,&mut la,&mut ad,true),program_account(&config_key,&program_id,&mut lc,&mut cd,true)];process_instruction(&program_id,&accounts,&set_root_data([9u8;32])).unwrap_err()};
 assert_eq!(err,ProgramError::MissingRequiredSignature);assert_eq!(cd,before);
}

#[test]
fn zero_trusted_merkle_root_is_rejected(){
 let program_id=Pubkey::new_unique();let authority=Pubkey::new_unique();let treasury=Pubkey::new_unique();let config_key=Pubkey::new_unique();let(mut lc,mut cd)=initialized_config(&program_id,&authority,&treasury,&config_key);let mut la=1;let mut ad=[];
 let err={let accounts=vec![program_account(&authority,&program_id,&mut la,&mut ad,true),program_account(&config_key,&program_id,&mut lc,&mut cd,true)];process_instruction(&program_id,&accounts,&set_root_data([0u8;32])).unwrap_err()};
 assert_eq!(err,ProgramError::Custom(WatcherError::InvalidPublicInputs as u32+1));assert_eq!(ConfigAccount::unpack(&cd).unwrap().merkle_root,[0u8;32]);
}
