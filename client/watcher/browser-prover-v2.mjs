...
class BrowserProverV2 {
  ...
  ensureWorker() {
    ...
    this.worker = new Worker(`${this.basePath}/worker.js`, { name: 'watcher-browser-prover-v2' });
    this.worker.onmessage = (event) => {
      ...
      if (message.type === 'error') pending.reject(new Error(message.error || 'V2 browser prover failed'));
      else pending.resolve(message.payload);
    };
    this.worker.onerror = (event) => {
      const error = new Error(event?.message || 'V2 browser prover worker crashed');
  ...
  async proveDeposit({ witness, expectedPublicInputs, onProgress } = {}) {
    ...
    try {
      ...
      return validateProofPayloadV1({
        ...
        expectedPublicInputBytes: DEPOSIT_PUBLIC_INPUT_BYTES_V2,
        source: 'V2 browser prover',
      });
    } finally {
      remove();
    }
  ...
  async proveWithdraw({ witness, expectedPublicInputs, onProgress } = {}) {
    ...
    try {
      ...
      return validateProofPayloadV1({
        payload,
        expectedPublicInputs,
        expectedPublicInputBytes: WITHDRAW_PUBLIC_INPUT_BYTES_V2,
        source: 'V2 browser prover',
      });
    } finally {
      remove();
...
export function getBrowserProverV2({ basePath = DEFAULT_BROWSER_PROVER_BASE_V2 } = {}) {
  const normalized = normalizeBasePath(basePath);
  let instance = instances.get(normalized);
  if (!instance) {
    instance = new BrowserProverV2(normalized);
